...
public class ProductShow {
	...
	public void showProduct() throws Exception {
		...
		System.out.println("                   상품설명 : " + dbprodContent);
		...
		switch (choiceNum) {
		case "1":
			bi.inputBasket(buyProduct(prodName, conn), conn);
			break;
		case "2":
			ms.insertGreat(findProduct(prodName, conn));
			break;
		}
	...
	public void showProductreview(String prodName, Connection conn) throws Exception {
		...
		String sql = "SELECT RPAD(RE_TITLE,20) , STAR_COUNT , RPAD(RE_CONTENT,23) FROM REVIEW R JOIN MEMBER M ON R.MEMBER_NO = M.MEMBER_NO JOIN PRODUCT P ON R.PRODUCT_NO = P.PROD_NO WHERE RE_DEL_YN = 'N' AND PROD_NAME = ? ORDER BY REVIEW_NO";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, prodName);
		ResultSet rs = pstmt.executeQuery();
		...
		while (rs.next()) {
			...
			System.out.print("        " + rs.getString("RPAD(RE_TITLE,20)"));
	...
	public String buyProduct(String proName, Connection conn) throws Exception {
		...
		System.out.println("          사이즈 선택(1. S / 2. M / 3. L / 4. XL) ");
		System.out.print("                       입력 >> ");
